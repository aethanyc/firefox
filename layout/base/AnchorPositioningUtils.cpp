...
// True if aFrame, or any frame up to (but not including) aAncestor, has a CSS
// transform.
static bool HasTransformUpTo(const nsIFrame* aFrame,
                             const nsIFrame* aAncestor) {
  for (const nsIFrame* f = aFrame; f && f != aAncestor; f = f->GetParent()) {
    if (f->IsTransformed()) {
      return true;
    }
  }
...
// Whether the anchor rect of aFrame needs to be mapped through CSS transforms
// to reach aContainingBlock. When this is false, callers must keep resolving
// the rect exactly as they did before transforms were taken into account, so
// that untransformed content is unaffected.
static bool ShouldApplyAnchorTransform(const nsIFrame* aFrame,
                                       const nsIFrame* aContainingBlock) {
  return StaticPrefs::
             layout_css_anchor_positioning_follows_transforms_enabled() &&
         HasTransformUpTo(aFrame, aContainingBlock);
}

// Maps aLocalRect, in aFrame's own coordinate space, into aAncestor's
// coordinate space, applying the CSS transform of aFrame and of every frame up
// to (but not including) aAncestor, while ignoring scroll offsets - anchor
// positioning compensates for scroll separately. Returns the axis-aligned
// bounding box.
static nsRect TransformRectToAncestorIgnoringScroll(const nsIFrame* aFrame,
                                                    const nsRect& aLocalRect,
                                                    const nsIFrame* aAncestor) {
  MOZ_ASSERT(nsLayoutUtils::IsProperAncestorFrame(aAncestor, aFrame));
...
Maybe<nsRect> AnchorPositioningUtils::GetAnchorPosRect(
  ...
  auto rect = [&]() -> Maybe<nsRect> {
    ...
    if (const nsIFrame* matchingCB =
            GetMatchingContainingBlock(aAnchor, aAbsoluteContainingBlock);
        matchingCB && ShouldApplyAnchorTransform(aAnchor, matchingCB)) {
      return Some(
...
auto AnchorPositioningUtils::GetCombinedFragmentRects(
  ...
  MOZ_ASSERT_IF(aApplyTransform == ApplyTransform::Yes, aContainingBlock);
  ...
  // Each fragment is mapped into aContainingBlock's space on its own when
  // applying transforms, so the union is formed there rather than in aFrame's
  // space, and needs no trailing offset.
  auto rectOf = [&](const nsIFrame* aFragment) {
    return applyTransform
               ? TransformRectToAncestorIgnoringScroll(
                     aFragment, aFragment->GetRectRelativeToSelf(),
                     aContainingBlock)
               : aFragment->GetRectRelativeToSelf() +
                     aFragment->GetOffsetTo(aFrame);
  };

  // Collect rects from our continuations (limited to those that are on the
  // same page if the context is paginated).
  nsRect rect = rectOf(aFrame);
  const auto* next = aFrame->GetNextContinuation();
  for (; next && onSamePage(next) && inSameCBFragment(next);
       next = next->GetNextContinuation()) {
    rect = rect.Union(rectOf(next));
  }
  const auto* prev = aFrame->GetPrevContinuation();
  for (; prev && onSamePage(prev) && inSameCBFragment(prev);
       prev = prev->GetPrevContinuation()) {
    rect = rect.Union(rectOf(prev));
  }
