...
struct ReflowInput : public SizeComputationInput {
  ...
  // Returns true if one of the following conditions satisfied: 1) mFrame has a
  // constrained available block-size; 2) mFrame has a multi-column ancestor
  // performing the last column balancing reflow; 3) mFrame is in a continuation
  // chain. When this method returns true, mFrame can be considered to be in a
  // "fragmented context."
  //
