...
void nsBlockFrame::ReflowAbsoluteDescendantsInInlineCB(
  ...
  for (auto& line : Lines()) {
    ...
    if (sawAbspos) {
      // Update the line's overflow areas to include absolute descendants'
...
void nsBlockFrame::ReflowAbsoluteFramesInInlineCB(
  ...
  // and stopping before this block. To update this block's overflow area, we'll
  // union absoluteOverflow with aReflowOutput.mOverflowAreas at the end of this
  // function.
  ...
  // block's coordinate space, and union it with the line's and aReflowOutput's
  // overflow area.
