...
// Return true if aInlineFrame (an absolute containing block laid out in
// aBlockFrame) continues into a next fragmentainer (column/page).
//
// We detect it by checking if aInlineFrame's last continuation or ib-split
// sibling is in aBlockFrame's overflow lines. Note: We can't rely on walking
// in-flow chain into a different fragmentainer here because the aBlockFrame's
// next-in-flow hasn't been created yet.
static bool IsInlineFrameContinuingInNextFragmentainer(
  ...
  const nsIFrame* inlineAcestorInLine =
      nsLayoutUtils::LastContinuationOrIBSplitSibling(aInlineFrame);
  while (inlineAcestorInLine &&
         inlineAcestorInLine->GetParent() != aBlockFrame) {
    inlineAcestorInLine = inlineAcestorInLine->GetParent();
  }
  if (inlineAcestorInLine &&
      overflowLines->mFrames.ContainsFrame(inlineAcestorInLine)) {
    return true;
  }
  return false;
}
