...
void nsBlockFrame::ReflowAbsoluteDescendantsInInlineFrame(
  ...
  for (auto& line : Lines()) {
    MOZ_ASSERT(!line.IsDirty(),
               "We should've reflowed all lines before we get here!");

    if (line.IsBlock()) {
      // The block frame in this line has reflowed its abspos descendants.
      continue;
    }
...
Maybe<OverflowAreas> nsBlockFrame::ReflowAbsoluteFramesInInlineFrame(
  ...
  MOZ_ASSERT(!nsLayoutUtils::GetPrevContinuationOrIBSplitSibling(aInlineFrame),
  ...
  OverflowAreas absposOverflow;
  absCB->Reflow(aInlineFrame, aPresContext, inlineRI, aStatus, cbRect, flags,
                &absposOverflow);
  return Some(absposOverflow);
}
