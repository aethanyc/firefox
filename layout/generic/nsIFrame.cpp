...
Matrix4x4Flagged nsIFrame::GetTransformMatrix(
  ...
  auto GetPositionPerFlag = [aFlags](const nsIFrame* aFrame) {
  ...
  if (isTransformed || zoomedContentRoot) {
    ...
    *aOutAncestor = GetParent();
    nsPoint delta = GetPositionPerFlag(this);
  ...
  *aOutAncestor =
  ...
  nsPoint offset = GetPositionPerFlag(this);
  ...
  while (*aOutAncestor != aStopAtAncestor.mFrame &&
    ...
    docOffset += GetPositionPerFlag(*aOutAncestor);
