...
void nsPageContentFrame::Reflow(nsPresContext* aPresContext,
  ...
  // Reflow our fixed pos frames if any. Note that we don't need to
  // PrepareAbsoluteFrames() because the fixed pos frames cannot split.
  if (absCB && absCB->HasAbsoluteFrames()) {
    // The containing block for the fixed pos kids is formed by our padding
    // edge.
