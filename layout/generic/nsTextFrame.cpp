...
bool TextAutospace::ShouldApplySpacing(CharClass aPrevClass,
  ...
  if (mBoundarySet.contains(Boundary::IdeographAlpha)) {
    if ((aPrevClass == CharClass::Ideograph &&
         aCurrClass == CharClass::NonIdeographicLetter) ||
        (aPrevClass == CharClass::NonIdeographicLetter &&
         aCurrClass == CharClass::Ideograph)) {
      return true;
    }
  }
  ...
  if (mBoundarySet.contains(Boundary::IdeographNumeric)) {
    if ((aPrevClass == CharClass::Ideograph &&
         aCurrClass == CharClass::NonIdeographicNumeral) ||
        (aPrevClass == CharClass::NonIdeographicNumeral &&
         aCurrClass == CharClass::Ideograph)) {
      return true;
    }
  }
    ...
    while (run.NextRun()) {
      ...
      for (int32_t i = 0; i < run.GetRunLength(); ++i) {
        ...
        if (mTextAutospace &&
          ...
          // It is rare that the current class is a combining mark, as combining
          // marks won't be a cluster starters. However, we still check it here
          // in case a stray mark is at the start of a frame.
