    ...
    if (aKidFrame->IsMenuPopupFrame()) {
      ...
      if (aDelegatingFrame->IsInlineFrameOrSubclass()) {
        ...
        if (unfragmentedPosition) {
          ...
          kidPos = *GetUnfragmentedPosition(aReflowInput,
                                            aKidFrame->FirstContinuation());
          kidPos.B(outerWM) = 0;
        }
        const LogicalSize kidSize = kidDesiredSize.Size(outerWM);
        LogicalRect kidRect(outerWM, kidPos, kidSize);
        nsRect kidPhysicalRect = kidRect.GetPhysicalRect(outerWM, blockSize);
        nsIFrame* inlineParent = aKidFrame->GetParent();
        nsIFrame* blockAncestor =
            nsLayoutUtils::FindNearestBlockAncestor(inlineParent);
        kidPhysicalRect -= inlineParent->GetOffsetTo(blockAncestor);
        aKidFrame->SetRect(kidPhysicalRect);
      } else {
        LogicalPoint kidPos(outerWM);
        if (unfragmentedPosition) {
          MOZ_ASSERT(!kidPrevInFlow, "aKidFrame should be a first-in-flow!");
          ...
          // aKidFrame is a next-in-flow. Place it at the block-edge start of
          // its containing block, with the same inline-position as its
          // prev-in-flow.
          kidPos = LogicalPoint(
