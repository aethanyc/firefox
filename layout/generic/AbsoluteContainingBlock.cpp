...
void AbsoluteContainingBlock::ReflowAbsoluteFrame(
  ...
  do {
    ...
    if (kidFrameMaySplit) {
      ...
      if (aKidFrame->HasProperty(nsIFrame::UnfragmentedPositionProperty())) {
        const nsSize cbBorderBoxSize =
        ...
        printf("kidPos.B %s, availBSize %d, mCumulativeAvailBSize %d\n",
               ToString(kidPos).c_str(), availBSize, mCumulativeAvailBSize);
    ...
    if (aKidFrame->IsMenuPopupFrame()) {
      ...
      printf("after reflow, kid pos %s\n",
             ToString(aKidFrame->GetProperty(
                          nsIFrame::UnfragmentedPositionProperty()))
                 .c_str());
