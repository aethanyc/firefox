...
// fragmentainer, i.e. no earlier continuation shares its nearest block
// ancestor.
static bool IsFirstInlineContinuationInFragmentainer(
  ...
  for (nsIFrame* prev =
    ...
    // If the previous inline continuation shares our fragmentainer, we are not
    // the first. Otherwise, it lives in a previous fragmentainer, so we are.
...
bool AbsoluteContainingBlock::PrepareAbsoluteFrames(
  ...
  // that hold abspos children in a later column can be moved into the current
