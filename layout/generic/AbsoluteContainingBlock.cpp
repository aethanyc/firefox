...
void AbsoluteContainingBlock::RemoveFrame(FrameDestroyContext& aContext,
  ...
  if (!StaticPrefs::layout_abspos_fragment_aware_positioning_enabled()) {
...
void AbsoluteContainingBlock::ReflowAbsoluteFrame(
  ...
  do {
    ...
    if (kidFrameMaySplit) {
      ...
      // For the kid's first-in-flow, subtract the border-block-start to reflow
      // it at the padding-edge.
      if (!kidPrevInFlow) {
    ...
    if (kidPrevInFlow && !kidPrevInFlow->IsMenuPopupFrame()) {
