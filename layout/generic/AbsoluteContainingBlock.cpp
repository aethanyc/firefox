    ...
    if (kidNeedsReflow && !aPresContext->HasPendingInterrupt()) {
      ...
      if (!kidFrameNeedsPush) {
        ...
        if (aReflowInput.mFlags.mIsInColumnMeasuringReflow) {
          ...
          // kidFrame must be a first-in-flow here: either from the first
          // measuring reflow where all frames are first-in-flows, or pulled
          // into this absolute containing block during an incremental reflow.
          // Any next-in-flows are in our pushed child list or in a later
          // continuation of this absolute containing block.
  ...
  do {
    ...
    if (kidFrameMaySplit) {
      if (unfragmentedPosition) {
        ...
        availBSize =
            aReflowInput.AvailableBSize() - (unfragmentedPosition->B(outerWM) -
                                             mCumulativeContainingBlockBSize);
