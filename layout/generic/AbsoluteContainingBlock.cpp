...
static bool IsFirstInlineContinuationInFragmentainer(
  ...
  if (prev && prev->IsBlockFrameOrSubclass()) {
    // Skip IB-split block siblings.
    prev = nsLayoutUtils::GetPrevContinuationOrIBSplitSibling(prev);
  }
  return prev ? nsLayoutUtils::FindNearestBlockAncestor(prev) != myBlock : true;
}

// Return the *first* (start-most) inline continuation in aInlineFrame's
// fragmentainer.
static nsIFrame* GetFirstInlineContinuationInFragmentainer(
    const nsIFrame* aInlineFrame) {
  MOZ_ASSERT(aInlineFrame->IsInlineFrameOrSubclass());
  const nsBlockFrame* myBlock =
      nsLayoutUtils::FindNearestBlockAncestor(aInlineFrame);
  nsIFrame* first = const_cast<nsIFrame*>(aInlineFrame);
  for (nsIFrame* prev =
           nsLayoutUtils::GetPrevContinuationOrIBSplitSibling(aInlineFrame);
    ...
    if (nsLayoutUtils::FindNearestBlockAncestor(prev) != myBlock) {
      break;
    }
    first = prev;
  }
...
// Return the *first* (start-most) inline continuation in aInlineFrame's
// previous fragmentainer. Or return nullptr if no such continuation in the
// previous fragmentainer or no previous fragmentainer.
static nsIFrame* GetFirstInlineContinuationInPrevFragmentainer(
    const nsIFrame* aInlineFrame) {
  MOZ_ASSERT(aInlineFrame->IsInlineFrameOrSubclass());
  nsIFrame* prev = nsLayoutUtils::GetPrevContinuationOrIBSplitSibling(
      GetFirstInlineContinuationInFragmentainer(aInlineFrame));
  if (prev && prev->IsBlockFrameOrSubclass()) {
    // Skip IB-split block siblings.
    prev = nsLayoutUtils::GetPrevContinuationOrIBSplitSibling(prev);
  }
  return prev ? GetFirstInlineContinuationInFragmentainer(prev) : nullptr;
}

// Return the *first* (start-most) inline continuation in aInlineFrame's next
// fragmentainer. Or return nullptr if no such fragment in the next
// fragmentainer or no next fragmentainer.
...
void AbsoluteContainingBlock::PullAbsoluteFramesFrom(
  ...
  for (auto iter = absCB->GetChildList().begin();
    ...
    // Advance the iterator first, so it's safe to move |child|.
    nsIFrame* const child = *iter++;
    if (aOnlyFirstInFlows == OnlyFirstInFlows::Yes && child->GetPrevInFlow()) {
      continue;
    }
...
bool AbsoluteContainingBlock::PrepareAbsoluteFrames(
  ...
  if (StaticPrefs::layout_abspos_fragment_aware_inline_cb_enabled() &&
    ...
    for (nsIFrame* next = nsLayoutUtils::GetNextContinuationOrIBSplitSibling(
      ...
      if (nsLayoutUtils::FindNearestBlockAncestor(next) != myBlock) {
        // Reached a continuation in the next fragmentainer.
        break;
      }
