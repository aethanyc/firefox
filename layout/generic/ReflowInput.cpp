  ...
  if (nullptr == mParentReflowInput || mFlags.mDummyParentReflowInput) {
    ...
    if (aContainingBlockSize.isNothing()) {
      MOZ_ASSERT(!mFrame->IsAbsolutelyPositioned(mStyleDisplay) ||
                     // XXX: We have a hack putting abspos continuations in
                     // overflow container lists (bug 154892), so they are not
                     // reflowed by AbsoluteContainingBlock until we revisit the
                     // abspos continuations handling.
                     mFrame->GetPrevInFlow(),
                 "AbsoluteContainingBlock always provides a containing block "
                 "size when creating ReflowInput for its children!");
