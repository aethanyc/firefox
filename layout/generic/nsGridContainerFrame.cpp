...
struct MOZ_STACK_CLASS nsGridContainerFrame::GridReflowInput {
  ...
  /**
   * In the given aAxis, reset the track sizing bits for all grid items, and
   * mark the track sizes and positions unresolved.
   *
   * This helper should be called before calling CalculateTrackSizesForAxis()
   * again in aAxis. Otherwise, there will be assertions.
   */
  void ResetTrackSizesForAxis(LogicalAxis aAxis);
...
void nsGridContainerFrame::GridReflowInput::ResetTrackSizesForAxis(
    LogicalAxis aAxis) {
...
LogicalSize nsGridContainerFrame::GridReflowInput::PercentageBasisFor(
  ...
  MOZ_ASSERT(!StaticPrefs::layout_css_grid_multi_pass_track_sizing_enabled(),
             "We go into legacy path when multi-pass track sizing enabled!");
...
void nsGridContainerFrame::Reflow(nsPresContext* aPresContext,
  ...
  if (MOZ_LIKELY(!prevInFlow)) {
    ...
    if (StaticPrefs::layout_css_grid_multi_pass_track_sizing_enabled()) {
      // Reset the track sizes before re-resolving the column sizes.
      gridRI.ResetTrackSizesForAxis(LogicalAxis::Inline);
      ...
      if (bSizeForResolvingRowSizes == NS_UNCONSTRAINEDSIZE &&
        ...
        bSizeForResolvingRowSizes = gridRI.mReflowInput->ApplyMinMaxBSize(
            gridRI.mRows.TrackSizeWithoutAlignment(this));
        ...
        // Reset the track sizes before re-resolving the row sizes.
        gridRI.ResetTrackSizesForAxis(LogicalAxis::Block);
